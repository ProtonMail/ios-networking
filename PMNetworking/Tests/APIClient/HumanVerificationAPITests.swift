...
class HumanVerificationAPITests: XCTestCase {
    ...
    override func setUp() {
        ...
        // get code stub
        stub(condition: isHost("protonmail.blue") && isMethodPOST() && isPath("/api/users/code")) { request in
            let body = request.ohhttpStubs_httpBody!
            let resultBody: Data
            do {
                let dict = try JSONSerialization.jsonObject(with: body, options: []) as! [String: Any]
                if let value = dict["Type"] as? String, (value == "email" || value == "sms"), let dest = dict["Destination"] as? Dictionary<String, Any> {
    ...
    func testSendCodeInvalidEmail() {
        ...
        api.exec(route: route) { (task, response) in
            XCTAssertEqual(response.code, 12221)
            XCTAssert(response.error != nil)
    ...
    func testSendCodeInvalidSMS() {
        ...
        api.exec(route: route) { (task, response) in
            XCTAssertEqual(response.code, 12231)
            XCTAssert(response.error != nil)
    ...
    func testHumanVerificationClose() {
        ...
        stub(condition: isHost("protonmail.blue") && isMethodPOST() && isPath("/api/internal/tests/humanverification") && !hasHeaderNamed("x-pm-human-verification-token")) { request in
    ...
    func testCaptchaMethodSuccess() {
        ...
        stub(condition: isHost("protonmail.blue") && isMethodPOST() && isPath("/api/internal/tests/humanverification") && hasHeaderNamed("x-pm-human-verification-token", value: captchaToken) && hasHeaderNamed("x-pm-human-verification-token-type", value: "captcha") && hasHeaderNamed("Content-Type")) { request in
    ...
    func testEmailMethodSuccess() {
        ...
        // backend answer with verification token and verification token type
        stub(condition: isHost("protonmail.blue") && isMethodPOST() && isPath("/api/internal/tests/humanverification") && hasHeaderNamed("x-pm-human-verification-token", value: emailToken) && hasHeaderNamed("x-pm-human-verification-token-type", value: "email") && hasHeaderNamed("Content-Type")) { request in
            let body = self.responseStringSuccess.data(using: String.Encoding.utf8)!
            let headers = ["Content-Type" : "application/json;charset=utf-8"]
            return HTTPStubsResponse(data: body, statusCode: 200, headers: headers)
        }
        ...
        let testHumanVerifyDelegate = HumanCheckHelperMock(apiService: api, resultSuccess: true, resultHeaders: [resultHeaders]) { responseResult in
            ...
            api.exec(route: route) { (task, response) in
                XCTAssertEqual(response.code, 1000)
                XCTAssert(response.error == nil)
                responseResult(response.code == 1000)
    ...
    func testSmsMethodSuccess() {
        ...
        // backend answer with verification token and verification token type
        stub(condition: isHost("protonmail.blue") && isMethodPOST() && isPath("/api/internal/tests/humanverification") && hasHeaderNamed("x-pm-human-verification-token", value: smsToken) && hasHeaderNamed("x-pm-human-verification-token-type", value: "sms") && hasHeaderNamed("Content-Type")) { request in
            let body = self.responseStringSuccess.data(using: String.Encoding.utf8)!
            let headers = ["Content-Type" : "application/json;charset=utf-8"]
            return HTTPStubsResponse(data: body, statusCode: 200, headers: headers)
        }
    ...
    func testMultipleRequests_1xVerify9001_5xVerify1000() {
        ...
        stub(condition: isHost("protonmail.blue") && isMethodPOST() && isPath("/api/internal/tests/humanverification") && !hasHeaderNamed("x-pm-human-verification-token") && { _ in isHumanVerificationRequired == true }) { request in
            let body = self.responseString9001.data(using: String.Encoding.utf8)!
            let headers = ["Content-Type" : "application/json;charset=utf-8"]
            return HTTPStubsResponse(data: body, statusCode: 200, headers: headers)
        }
        ...
        stub(condition: isHost("protonmail.blue") && isMethodPOST() && isPath("/api/internal/tests/humanverification") && !hasHeaderNamed("x-pm-human-verification-token") && { _ in isHumanVerificationRequired == false }) { request in
            let body = self.responseStringSuccess.data(using: String.Encoding.utf8)!
            let headers = ["Content-Type" : "application/json;charset=utf-8"]
            return HTTPStubsResponse(data: body, statusCode: 200, headers: headers)
        }
